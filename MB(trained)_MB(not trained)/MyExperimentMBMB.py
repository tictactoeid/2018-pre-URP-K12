...
# ==========================================================================================
import MyPongMBMB # My PyGame Pong Game 
import MyAgentMBMB # My DQN Based Agent

import numpy as np 
import random 
import matplotlib.pyplot as plt
import MyAgentMBMBsub_brain
import csv
...
# =======================================================================
# Normalise GameState
TheAgentsub1 = MyAgentMBMBsub_brain.Agent(2, 1)
def CaptureNormalisedState(PlayerYPos, BallXPos, BallYPos, BallXDirection, BallYDirection, BallSpeed):
	...
	gstate[5] = TheAgentsub1.ballysub(gstate1)
        ...
        return gstate
TheAgentsub2 = MyAgentMBMBsub_brain.Agent(2, 1)
def CaptureNormalisedState2(PlayerYPos, BallXPos, BallYPos, BallXDirection, BallYDirection, BallSpeed):
	...
	gstate[0] = PlayerYPos/400.0    # Normalised PlayerYPos
        ...
        gstate[3] = BallXDirection*BallSpeed  # Normalised BallXDirection
        gstate[4] = BallYDirection*BallSpeed  # Normalised BallYDirection
	gstate[5] = TheAgentsub2.ballysub(gstate1)
...
def PlayExperiment():
        ...
        TheGame = MyPongMBMB.PongGame()
    # Initialise Game
        TheGame.InitialDisplay()
        #
        #  Create our Agent (including DQN based Brain)
        TheAgent1 = MyAgentMBMB.Agent(STATECOUNT+1, ACTIONS)
        TheAgent1.Load(10000000)
	TheAgentsub1.Load(10000000)
        TheAgent2 = MyAgentMBMB.Agent(STATECOUNT+1, ACTIONS)
        GameTime = 0
        ...
        GameState1 = CaptureNormalisedState(200.0, 200.0, 200.0, 0, 0, 1.0)
    ...
    # =================================================================
        ...
        while (GameTime<TOTAL_GAMETIME):
                ...
                NextState1 = CaptureNormalisedState(Player1YPos, BallXPos, BallYPos, BallXDirection, BallYDirection, BallSpeed)
        ...
        #print our where wer are after saving where we are
                if GameTime % 500000 == 0:
                        doNothing = 0
			TheAgentsub2.Save(GameTime)
                        TheAgent2.Save(GameTime)
                ...
                if GameTime % 1200 == 0:
                        ...
                        print("Game Time: ", GameTime,"  Game Score1: ", "{0}".format(TheGame.Display_Score1),"  Game Score2: ", "{0}".format(TheGame.Display_Score2)," Recent Rate: ","{0}:{1}".format(s,MyPongMBMB.RECENT_SCORE-s) )
