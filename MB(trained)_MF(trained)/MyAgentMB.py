...
class Brain:
        ...
        def train(self, x, y, epoch=1, verbose=0):
                self.model.fit(x, y, batch_size=BATCH_SIZE, epochs=epoch, verbose=verbose, initial_epoch = epoch-1)
...
class Agent:
        ...
        # ============================================
        def Load(self, num):
                self.brain.model = keras.models.load_model("./10M_MB/testModel{}".format(num))
                with open("./10M_MB/testModel{}.steps".format(num),"r") as f:
                        self.steps = int(f.read())
                        print(self.steps)
                #with open("./10M_MB/testModel{}.exp".format(num),"rb") as f:
                        #self.ExpReplay.samples =  pickle.load(f) #학습 중간에 멈췄다가 로딩할 때만 필요
        ...
        # ============================================
        def Save(self, num):
                self.brain.model.save("./model/testModel{}".format(num))
                with open("./model/testModel{}.steps".format(num),"w") as f:
                        f.write('{}'.format(self.steps))
                        print("Write!")
                        print(self.steps)
                        
                with open("./model/testModel{}.exp".format(num),"wb") as f:        
